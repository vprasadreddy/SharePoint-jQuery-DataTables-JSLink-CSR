...
(function () {
    ...
    itemCtx.ListTemplateType = 101; /* This Template ID is for SharePoint Document Library */
    ...
    itemCtx.OnPostRender.push(function()
    {
		...
		*/
        ...
        buttons: [
			...
			*/
			{
            text: '<i class="fa fa-upload" style="font-size:15px" aria-hidden="true"></i> Upload a document',
            action: function ( e, dt, node, config ) 
				{
				var uploadURL = ctx.HttpRoot + "/_layouts/Upload.aspx?&List=" + ctx.listName + "&Source=" + ctx.ListSchema.PagePath;
				/* 
				use window.open(uploadURL); to open document upload dialog box in new window. 
				use javascript:ModalDailog(uploadURL); to open document upload dialog box in SharePoint dialog box.
				*/
				javascript:ModalDailog(uploadURL);
				}
			},
            {
			extend:'copyHtml5',
...
function ItemOverrideDataTables(ctx) {
    ...
    for (index = 0, len = ctx.ListSchema.Field.length; index < len; ++index) {
		...
		/* To check if the Column is of type LookUP with multiple values*/
		...
		/* check for Edit option */
			...
			** To open EditForm in a new tab.
			change ctx.editFormUrl to your URL variable or actual "URL value" enclosed in double quotes.
			"<a href='" + ctx.editFormUrl + "&amp;ID=" + ctx.CurrentItem.ID + "&Source=" + ctx.ListSchema.PagePath +"' target="+ '_blank' +">";
			
			*** ctx.ListSchema.PagePath is the URL of the current view from which the user is editing the document properties. ***
			*/
		...
		/* check for item Link */
        else if (ctx.ListSchema.Field[index].Name === "LinkFilename") 
		{
			
			var fileName = ctx.CurrentItem["FileLeafRef"];
			/* This Regex expression use to delete extension (.docx, .pdf, .pptx, .txt ....etc) form the file name  */
			fileName = fileName.replace(/\.[^/.]+$/, "") 
            cell = "<a href='" + ctx.CurrentItem.FileRef + "'>";
            cell += fileName;
            cell += "</a>";
        }
		/* end of item Link */
		...
		*/
		...
		{
			/* condition to check if the people picker has only one value*/
			if (ctx.CurrentItem[ctx.ListSchema.Field[index].RealFieldName].length == 1) 
			{
				for (index1 = 0, len1 = ctx.CurrentItem[ctx.ListSchema.Field[index].RealFieldName].length; index1 < len1; ++index1) {
                cell += ctx.CurrentItem[ctx.ListSchema.Field[index].RealFieldName][index1].title + " ";
...
/* This function is used to refresh the page after a document is uploaded successfully. */
...
/* END of function to refresh the page after a document is uploaded successfully. */
</script>
